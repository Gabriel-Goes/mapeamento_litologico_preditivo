...
# + busca Sentinel-2 L2A (BDC STAC) para fusão
...
# Sentinel-2 L2A no BDC
S2_STAC_URL = "https://data.inpe.br/bdc/stac/v1"
S2_COLLECTION_ID = "S2_L2A_BUNDLE-1"
...
# ----------------------------------------------------------------------
# Sentinel-2 L2A (BDC STAC) helpers
# ----------------------------------------------------------------------
def _parse_s2_datetime(props: Dict[str, Any]) -> Optional[datetime]:
    dt_str = (
        props.get("datetime")
...
def search_sentinel2_granules(
    ...
    """
    Busca cenas Sentinel-2 L2A (BDC, coleção S2_L2A_BUNDLE-1) para a folha,
    ordenando por (nuvem crescente, |data - target_date|).
    """
    log("[S2] Iniciando search_sentinel2_granules (BDC STAC)")

    _, bbox = _get_folha_geom_and_bbox(folha_id)
    log(f"[S2] folha_id       = {folha_id}")
    log(f"[S2] collection     = {S2_COLLECTION_ID}")
    log(f"[S2] bbox (W,S,E,N) = {bbox}")
    log(f"[S2] intervalo      = ({start_date}, {end_date})")
    log(f"[S2] max_items      = {max_items}")
    log(f"[S2] cloud_max      = {cloud_max}")
    ...
    url = S2_STAC_URL.rstrip("/") + "/search"
    log(f"[S2] GET {url}")
    log(f"[S2] params = {params}")

    r = requests.get(url, params=params, timeout=120)
    log(f"[S2] HTTP {r.status_code}")
    r.raise_for_status()
    js = r.json()

    feats = js.get("features", [])
    log(f"[S2] Itens retornados: {len(feats)}")

    tgt_dt = datetime.fromisoformat(target_date)
    if tgt_dt.tzinfo is not None:
        tgt_dt = tgt_dt.replace(tzinfo=None)

    candidatos: List[Dict[str, Any]] = []

    for ft in feats:
        props = ft.get("properties", {}) or {}
        dt = _parse_s2_datetime(props)
        if dt is None:
            log("[S2] Item sem datetime parsável; ignorando.")
            continue
        ...
        delta_days = abs((dt_naive - tgt_dt).days)

        assets = ft.get("assets", {}) or {}
        preview_url: Optional[str] = None
        ...
        for key, asset in assets.items():
            ...
            low = href.lower()
            if preview_url is None and (low.endswith(".jpg") or low.endswith(".jpeg") or low.endswith(".png")):
                preview_url = href
            if tif_href is None and (low.endswith(".tif") or low.endswith(".tiff")):
                tif_href = href

        candidatos.append(
            {
                "item": ft,
                "collection": ft.get("collection", S2_COLLECTION_ID),
                "item_id": ft.get("id", ""),
                "datetime": dt_naive,
                "cloud": cloud,
    ...
    if not candidatos:
        log("[S2] Nenhum candidato com metadata utilizável (após filtro de nuvem).")
        return []

    def _score_s2(c: Dict[str, Any]) -> Tuple[float, int]:
        ccloud = c["cloud"]
        if ccloud is None:
            ccloud = 9999.0
        return float(ccloud), int(c["delta_days"])

    candidatos.sort(key=_score_s2)

    log("[S2] Cenas encontradas (ordenadas por nebulosidade e proximidade da data alvo):")
    for i, c in enumerate(candidatos[:10]):
        dt_iso = c["datetime"].isoformat()
        cloud_str = "None" if c["cloud"] is None else f"{c['cloud']:.1f}"
        log(
            f"  [{i}] id={c['item_id']} datetime={dt_iso} "
            f"cloud={cloud_str} delta_days={c['delta_days']} "
            f"preview={'OK' if c['preview_url'] else 'None'} "
            f"tif={'OK' if c['tif_href'] else 'None'}"
        )
...
class AsterEarthdataDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self._s2_candidates: List[Dict[str, Any]] = []
        ...
        self.btnSearch = QtWidgets.QPushButton("Listar cenas ASTER")
        self.btnPreview = QtWidgets.QPushButton("Preview ASTER selecionada")
        self.btnDownloadClip = QtWidgets.QPushButton("Baixar & recortar ASTER selecionada")
        self.btnSearchS2 = QtWidgets.QPushButton("Buscar Sentinel-2 L2A p/ cena ASTER selecionada")
        ...
        bottom.addWidget(self.btnSearchS2)
        bottom.addStretch(1)
        ...
        self.btnSearchS2.clicked.connect(self._on_search_s2_for_aster)
    ...
    def _on_search(self):
        ...
        QtWidgets.QMessageBox.information(self, "Busca ASTER", f"{len(cands)} cena(s) ASTER listada(s).")
    ...
    def _on_search_s2_for_aster(self):
        if not self._candidates:
            QtWidgets.QMessageBox.information(self, "Sentinel-2", "Liste primeiro cenas ASTER.")
            return
        idx_str = self._selected_row_idx()
        if idx_str is None:
            QtWidgets.QMessageBox.information(self, "Sentinel-2", "Selecione uma cena ASTER na tabela.")
            return
        try:
            idx = int(idx_str)
        except ValueError:
            QtWidgets.QMessageBox.information(self, "Sentinel-2", "Índice inválido.")
            return
        if idx < 0 or idx >= len(self._candidates):
            QtWidgets.QMessageBox.information(self, "Sentinel-2", "Índice fora da faixa.")
            return

        folha_id = self.edFolha.text().strip()
        if not folha_id:
            QtWidgets.QMessageBox.warning(self, "Sentinel-2", "Informe o código da folha.")
            return

        cand = self._candidates[idx]
        dt_aster = cand.get("datetime")
        if not isinstance(dt_aster, datetime):
            QtWidgets.QMessageBox.information(self, "Sentinel-2", "Cena ASTER sem datetime válido.")
            return

        target_date = dt_aster.date().isoformat()
        start_date = self.edStart.date().toString("yyyy-MM-dd")
        end_date = self.edEnd.date().toString("yyyy-MM-dd")
        cloud_max = float(self.spCloud.value())
        max_items = int(self.spLimit.value())
        ...
        try:
            s2_cands = search_sentinel2_granules(
                folha_id=folha_id,
            ...
            QtWidgets.QMessageBox.critical(self, "Sentinel-2", str(e))
            return

        self._s2_candidates = s2_cands

        if not s2_cands:
            QtWidgets.QMessageBox.information(self, "Sentinel-2", "Nenhuma cena Sentinel-2 encontrada.")
            return

        log("[S2] Melhores cenas Sentinel-2 para fusão:")
        lines = []
        for i, c in enumerate(s2_cands[:10]):
            dt_iso = c["datetime"].isoformat()
            cloud_str = "None" if c["cloud"] is None else f"{c['cloud']:.1f}"
            line = f"[{i}] {c['item_id']}  data={dt_iso}  nuvem={cloud_str}  Δdias={c['delta_days']}"
            log("  " + line)
            lines.append(line)

        msg = "\n".join(lines)
        QtWidgets.QMessageBox.information(self, "Sentinel-2 (BDC)", msg)
