...
# -----------------------------------------------------------------------------
def regions(malha_cartog,crs__='32724'):
    bounds = malha_cartog.bounds
    malha_cartog['region'] = [(left, right, bottom, top) for
                               left, right, bottom, top in
                               zip(bounds['minx'],bounds['maxx'],
                                   bounds['miny'],bounds['maxy'])]
    malha_cartog.to_crs("EPSG:"+crs__, inplace=True)
    print(f"-->{malha_cartog.crs}")
    bounds = malha_cartog.bounds
    malha_cartog['region_proj'] = [(left, right, bottom, top) for 
                                    left, right, bottom, top in 
                                    zip(bounds['minx'], bounds['maxx'],
                                        bounds['miny'], bounds['maxy'])]
    return malha_cartog
# -----------------------------------------------------------------------------
def cartas(escala='50k', ids=['SB24_ZA_III2'], crs__='32724'):
    '''
    Recebe:
        escala  : Escala das folhas cartográficas 
        ids     : Código da Folha Cartográfica
    '''
...
def set_region(escala, id, geof, camada, mapa=None,crs__=32723):
    '''
    Recebe:
    ...
    dict_cartas, mc_select = cartas(escala, id,crs__)
