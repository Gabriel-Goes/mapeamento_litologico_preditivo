...
def interpolar(mag=None, gama=None, geof=None, dic_cartas=None, dic_raw_meta=None):
    dic_cartas['cubic'] = {}
    ...
    for index, row in tqdm(dic_raw_meta['Malha_cartografica'].iterrows()):
        ...
        if len(data) == 0:
            ...
            shape = ((int((area[1]-area[0])/100),
                     int((area[3]-area[2])/100)))
                                
            # creating a grid with cells
            xu, yu = td.regular(area=area,shape=shape)
            if geof:
                print('dado geof')
                ...
                data_interpolado = {'X':xu, 'Y':yu, 'MAGIGRF': magr_,'ALTE':alte_,
                        'CTC': ctc_, 'KC': kc_, 'UC':uc_, 'THC': thc_} 
               # Atualizando chave 'cubic' com dados interpolados
                dic_cartas['cubic'].update({index:pd.dataframe(data_interpolado)})
            ...
            if mag:
                ...
                data_interpolado = {'X':xu, 'Y':yu, 'MDT': mdt_,
                        'ALTURA': altura_, 'MAGIGRF':magigrf_} 
