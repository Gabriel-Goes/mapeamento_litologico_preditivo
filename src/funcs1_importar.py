...
def importar_geometrias(camada, mapa=None):
    '''
    Recebe:
    ...
    print(f'{camada} lida do Geopackage')
    lito =  gpd.read_file(gdb,
            driver= 'GPKG',
            layer= camada)
    ...
    if mapa:
        ...
        if folha.empty:
            print('!Error!')
            print("A string pertence à coluna MAPA da camada vetorial.")
            print("Os mapas disponíveis são os listados a seguir:")
            print(f"# -- Lista de mapas: {list(lito.MAPA.unique())}")
            
            print('')
            print('folha = lito[lito.MAPA == "Carta geológica da folha " + <mapa>]')
            print('# Selecione apenas os caracteres após a palavra "folha"')
...
def import_malha_cartog(escala,ids=None):
    ...
    if ids:
        ...
        malha_cartog_contains_id = [malha_cartog['id_folha'].str.contains(ids)] # '.contains' não é ideal.
