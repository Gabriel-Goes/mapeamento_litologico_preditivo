...
def import_xyz(filename,rowskiped):
    data_cols = 'KB DATA BARO UB THB COSMICO CTB UUP ALTURA KPERC eU eTH CTEXP UTHRAZAO X Y UKRAZAO MDT THKRAZAO LIVE_TIME CTCOR KCOR THCOR UCOR HORA GPSALT LATITUDE FIDUCIAL TEMP LONGITUDE'.split(" ")

    geof = pd.read_csv(set_gdb('xyz/'+ filename),
                         names = data_cols,
                         delim_whitespace = True,
                         skiprows = rowskiped, # Linhas de cabeçalho
